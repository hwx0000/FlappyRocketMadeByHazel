...
void Renderer2DTestLayer::OnUpdate(const Hazel::Timestep& ts)
{
	...
	{
		Hazel::Renderer2D::DrawQuad({ 0, 0, -0.1 }, 0.0f, { 1.5, 1.5 }, { 1.0f, 0.0f, 0.0f, 1.0f });// 调用Renderer2D里的FlatColorShader
		Hazel::Renderer2D::DrawQuad({ 0, 0, 0 }, 0.0f, { 1.0, 1.0 }, m_Texture2D);// 调用Renderer2D里的TextureShader
	}
